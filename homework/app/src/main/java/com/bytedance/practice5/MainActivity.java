...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId){
        new Thread(new Runnable() {
            @Override
            public void run() {
                MessageListResponse constants = UrlGetConstain(studentId);
                if (constants != null && constants.success == true){
                    List<com.bytedance.practice5.model.Message> data = constants.feeds;
    ...
    public MessageListResponse UrlGetConstain(String studentId) {
        String path = "/messages";
        String urlStr;
        if (studentId == null){
            urlStr = String.format(Constants.BASE_URL+path);
        }else{
            urlStr = String.format(Constants.BASE_URL+path+"?student_id=%s",studentId);
        }
